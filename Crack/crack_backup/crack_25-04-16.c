...
#define P_KAPPA "damage interface energy kappa"
...
const double crack_thres = 0.50;						// below this value, material is considered as cracked and the mesh is refined
...
void add_ellipse_to_Poly(point2D** Poly,int* len,point2D* Center,double a,double b,int num,double (*Distr)(int i,int size)){
	...
	*Poly = (point2D*)realloc(*Poly,(*len+num)*sizeof(point2D));
	...
	for (i=1;i<=num;i++){
		s = (*Distr)(i,num);
		x = cos(s*angle)>=0 ? pow(fabs(cos(s*angle)),0.5)*a : -pow(fabs(cos(s*angle)),0.5)*a;
		y = sin(s*angle)*b;
		(*Poly)[*len+i-1] = init_point2D(Center->x+x,Center->y+y);
	}
	*len += num;
...
double create_shape_hexagons(char* name,int mesh_size){
	...
	if (Cond_file!=NULL) Bound_o[0] = P2;
	...
	if (Cond_file!=NULL) Bound_o[1] = P2;
	...
	if (Cond_file!=NULL) Bound_o[2] = P2;
	...
	if (Cond_file!=NULL) Bound_o[3] = P1;	
	...
	for (i=0;i<2*N;i++){
		for (j=0;j<M;j++){
			...
			if (Cond_file!=NULL) Bound_i[k-1] = P1;	
...
double* filtered_total_strain_trace(double* U,double* H,double* C, double* T){
	...
	int k;
	int n = glob_mesh.size;
	double K = sqrt(kappa/(2*xi))*factor;
...
void refine_by_strain_energy(double* X,double* Refine,double a_min){
	...
	int n = glob_mesh.size;
	double min_area = a_min*get_total_area();
	double* U = &(X[0]);
	...
	double exp10 = get_histogram_thres(Hist,histogram_resolution,strain_ref_max_area,strain_ref_alpha_factor);
	if (isnan(exp10)){
	...
	for (i=0;i<n;i++) if (fabs(Wel[i])>thres && fabs(Refine[i])>min_area) Refine[i] = min_area;
...
void refine_by_damage_predictor(double* X,double* Refine,double a_min){
	...
	if (xi==0){
		...
		for (i=0;i<n;i++){		
			if (Hpred[i]<0 && H[i]>Htol) Refine[i] = a_min*total_area;	
		}
	}
	else{
		Hpred = secondorder_predictor(U,H,C,T);
		for (i=0;i<n;i++){		
			if (Hpred[i]<(Hthres2-1.)) Refine[i] = a_min*total_area;	
		}
...
void refine_by_future_value(double* H_future,double* Refine,double a_min){
	...
	if (H_future!=NULL){
		...
		double l = sqrt(2.*total_area*min_refine_area);
		double range = 5.*l;
		...
		for (i=0;i<n;i++) if (Min[i]<Hthres) Refine[i] = a_min*total_area;
		free(Min);
...
void refine_by_damage_value(double* X,double* Refine,double a_min){
	...
	for (i=0;i<n;i++){	
		if (H[i]<Hthres) Refine[i] = a_min*total_area;
	}
}
...
void refine_by_damage_change(int time_index,double* Refine,double a_min){
	...
	if (time_index>1){	
		...
		double total_area = get_total_area();
		for (i=0;i<n;i++) if (fabs(dH[i])>thr_c) Refine[i] = a_min*total_area;
...
void print_moduli(FILE* file,char* var){
	...
	/*if (strcmp(P_MU1,var)==0){
		E = &Moduli_1;
		fprintf(file,"%s\t(xxxx=%e,yyyy=%e,xxyy=%e,xyxy=%e,xxxy=%e,yyyx=%e,reg=%e)\n"
			,P_MU1,E->xxxx,E->yyyy,E->xxyy,E->xyxy,E->xxxy,E->yyyx,E->reg);
	}
	if (strcmp(P_MU2,var)==0){
		E = &Moduli_2;
		fprintf(file,"%s\t(xxxx=%e,yyyy=%e,xxyy=%e,xyxy=%e,xxxy=%e,yyyx=%e,reg=%e)\n"
			,P_MU2,E->xxxx,E->yyyy,E->xxyy,E->xyxy,E->xxxy,E->yyyx,E->reg);
	}*/	
}
...
void Assign_param_strings(char** Names,char** Values,int len){
	...
	if (len>=P_COUNTER){
		for (i=0;i<len;i++){
			...
			else if (strcmp(Names[i],P_KAPPA)==0) kappa = atof(Values[i]);
			...
			else if (strcmp(Names[i],P_TIMESTEP_NUM)==0) time_steps = atoi(Values[i]);
...
void Read_console_parameters(int argc,char* argv[]){
	...
	for (i=Start;i<argc;i++) if (argv[i][0]=='-'){
		...
		if (len>1){
			...
			else if (strcmp(Line[0],PS_KAPPA)==0) {kappa = atof(Line[1]);printf("set %s: %e\n",P_KAPPA,kappa);}
			...
			else if (strcmp(Line[0],PS_TIMESTEP_NUM)==0) {time_steps = atoi(Line[1]);printf("set %s: %d\n",P_TIMESTEP_NUM,time_steps);}
...
void save(double* Sol,double* Globals,int glob_size,int t_index){
	...
	if (Sol!=NULL){
		...
		double* Alpha = damage_rate_independent(C);
		copy_vector_content(Alpha,AF,0,2*n,n);
		free(Alpha);
...
double damage_functional(double* X,int n){
	if (Strain_energy!=NULL && Glob_Pointer_C!=NULL && Glob_Pointer_H0!=NULL){
		...
		//sparse_mult(Interface_damage,H);
		//linear_map(S,-2.,Dissipation,Alpha);
		//res = dt*scalar(H,S,n)/2.;
...
double* damage_gradient(double* X){
	if (Strain_energy!=NULL && Glob_Pointer_C!=NULL && Glob_Pointer_H0!=NULL){
		...
		//sparse_mult(Interface_damage,X);		
		scalar_mult(dt*kappa,Grad,n);
...
sparse_matrix* get_damage_Hesse_linear(double* C){
	...
	double* Kappa = interface_energy_grad(C);
	sparse_matrix* Grad = sparse3D_vB(T_Flux,Kappa,n);
	//sparse_add(Res,Interface_damage,dt);										// rate independent part
	sparse_add(Res,Grad,kappa*dt);
...
double* damage_predictor(double* U,double* H,double* C,double* T){
	...
	double* K = sparse3D_vvB(T_Flux,Kappa,H,n);//sparse_mult(Interface_damage,H);
	scalar_mult(kappa,K,n);
	double* S = damage_self_gradient(H,C,n);
...
void setup(int argc, char* argv[],double** Initial, void (*Init_func)(double* X)){
	...
	if (round(total_time/dt)!=time_steps){
		dt = (double)total_time/time_steps;
...
void get_full_damage_system(sparse_matrix** A,double** b,double* C,double* H,double* H_prev,double* U,
 double* T,bound_cond* BC_list_U){
	...
	sparse_matrix* Int = sparse3D_vB(T_Flux,Kappa,n);	
...
void solve(double* Initial_data,double* Globals,double time,int steps){
	int i,j,n,n_prev,accepted,last_accepted,loc_iter;
	double u;
	double* Prev;
	double* X;
	double* M;
	...
	time_steps = 1;
	glob_time_index = 0;
	total_maxiter = 10*steps;
	dt_max = (double)time/steps;
	...
	do{
		...
		if (space_adaptive==YES){
			...
			refine_by_damage_value(X,Refine_data,min_refine_area/10.);		
			refine_by_strain_energy(X,Refine_data,min_refine_area/10);
		...
		// runtime info
		printf("\rtime: %.7f max: %f",glob_time,time+time_offset);		
		fflush(stdout);		
...
int main(int argc, char* argv[]){
	...
	if (strcmp(argv[1],"mesh")==0){						
		...
		if (Cond_file!=NULL) fclose(Cond_file);
