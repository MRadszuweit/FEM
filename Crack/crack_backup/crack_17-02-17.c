...
static matrix2D interface_tensor1 = {.xx=0.625,.xy=0.375,.yx=0.375,.yy=0.625};
static matrix2D interface_tensor2 = {.xx=0.625,.xy=0.375,.yx=0.375,.yy=0.625};
...
double* free_energy_damage_interface_dc(double* H,double* C){
	...
	vector_add(Res,contr,kappa/2.,n);
...
double damage_functional(double* X,int n){
	if (Strain_energy!=NULL && Glob_Pointer_C!=NULL && Glob_Pointer_H0!=NULL){
		...
		res = dt*kappa*vector_contraction(S,n)/2.;
...
double Free_energy_damage(double* H,double* C){
	...
	double res = kappa*vector_contraction(Res,n)/2.;
