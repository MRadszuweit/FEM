...
void InorderTreeCount(rb_red_blk_tree* tree, rb_red_blk_node* x,int* counter) { 
  if (x != tree->nil) {
    InorderTreeCount(tree,x->left,counter);
    (*counter)++;
    InorderTreeCount(tree,x->right,counter);
  }
}

int RBTreeCount(rb_red_blk_tree* tree){
	int res = 0;
	InorderTreeCount(tree,tree->root->left,&res);
	return res;
}
...
/*  FUNCTION:  RBLargest */
/**/
/*    INPUTS:  tree is the tree to search in  */
/**/
/*    OUTPUT:  returns the info of the node with largest key  */
/**/
/*    EFFECT:  deletes node with largest key */
/**/
/*    Modifies Input: none */
/**/
/***********************************************************************/

rb_red_blk_node* RBLargest(rb_red_blk_tree* tree) {
    rb_red_blk_node* nil=tree->nil;
    rb_red_blk_node* x=tree->root->left;
    if (x!=nil){
        while (x->right != nil){
