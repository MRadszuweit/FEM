...
static int RN_debug_mode = RN_NO;
...
void RN_set_trust_limits(double min,double max){
	RN_trust_radius_min = min;
	RN_trust_radius_max = max;
}
...
void subspace_minimum(sparse_matrix* A,double* b,double* V1,double* V2,double* X){  //  with constraint ||X||=RN_Delta
	...
	int root_num = Interval_Separation_Newton_1D(&RN_derivative,&Roots,0,2.*M_PI,Newton_iter,Newton_eps,intervals);
	
	/*double* Z = NULL;
	int n = Interval_Separation_Newton_1D(&RN_derivative,&Z,0,2.*M_PI,Newton_iter,Newton_eps,intervals);
	if (n!=root_num){
		printf("\nnormal Newton: %d roots, Interval Newton %d roots\n",root_num,n);
		printf("Newton roots:\n");
		for (i=0;i<root_num;i++) printf("%f\n",Roots[i]);
		printf("Interval Newton roots:\n");
		for (i=0;i<n;i++) printf("%f\n",Z[i]);
		printf("\n");
		fflush(stdout);
		RN_print_function(&RN_derivative,intervals,0,2*M_PI);		
	}
	if (Z!=NULL) free(Z);*/
...
double predicted_reduction(double* S,double* Gradient){
	return scalar(Gradient,S,RN_dim)+RN_min(sparse_bilinear(S,RN_Hesse,S),0);
}

void delete_small_diff(double* X,double eps){
	int i,index;
	
	for(i=0;i<RN_bound_num;i++){
		index = RN_bound[i];
		if (!isnan(RN_upper[i]) && fabs(X[index]-RN_upper[i])<eps) X[index] = RN_upper[i];
		if (!isnan(RN_lower[i]) && fabs(X[index]-RN_lower[i])<eps) X[index] = RN_lower[i];		
	}
}

/*int test_small(double* X){
	const double eps = 1e-30;
	int i;
	for (i=0;i<RN_dim;i++){
		if (X[i]!=0 && fabs(X[i])<eps){
			printf("very small value at i=%d: %e\n",i,X[i]);
			return 1;
		}
	}
	return 0;
...
int is_at_boundary(double* X){
	...
	for(i=0;i<RN_bound_num;i++){
		index = RN_bound[i];
		if (!isnan(RN_upper[i]) && fabs(X[index]-RN_upper[i])<eps) res = 1;
		if (!isnan(RN_lower[i]) && fabs(X[index]-RN_lower[i])<eps){
			res = 1;		
			//printf("small value at index: %d value: %e\n",index,X[index]);
		}
	}
...
void RN_iterate(sparse_matrix* M,double* Gradient,double* D,double* X){
	...
	int iter;
	double d,alpha,alpha_l,alpha_r,contraction,pred;
	double* S = NULL;
	...
	if (!RN_test_sigma_l(X_old,X,S,Gradient,alpha)){	// line search 			(piecewise linear bisection algorithm)
		...
		if (RN_info==RN_YES && iter==max_iter){
			printf("Reflective Newton warning: line search did not converge: alpha=%e\n",alpha);			
		}
	...
	pred = predicted_reduction(S,Gradient);
	contraction = d/pred;
...
int reflective_newton(double* X){
	...
	int iter = 0;
	double q,dq;
	double* Q = zero_vector(RN_max_iter);
	...
	do{		
		if (RN_debug_mode==RN_YES){
			char out[512];
			sprintf(out,"/Home/damage/radszuwe/Daten/RN%d",iter);
			print_scalar_data(out,X,RN_dim);			
		}
	...
	if (iter==RN_max_iter){
		printf("Reflective Newton: no convergence after %d iterations\n",iter);
		print_vector(Q,RN_max_iter);
	}
	statistics.res += dq;
	statistics.total_iter += iter;
	statistics.calls++;
	
	delete_small_diff(X,1e-3*RN_eps);
	
	if (RN_compute_Hesse!=NULL && RN_Hesse!=NULL){
		free_sparse(RN_Hesse);
		RN_Hesse = NULL;
	}
	free(Q);
...
int active_set_method(sparse_matrix* A,double* X,double* b,sparse_matrix* Constraint_Matrix,double definiteness,ass_info** info){
	int iter = active_set_solver(A,X,b,Constraint_Matrix,RN_lower,RN_upper,RN_multifrontal_solver,RN_max_iter,definiteness,ASS_DEFAULT,info);
	if (iter==RN_max_iter) printf("Active set method: no convergence after %d iterations\n",iter);
	return iter;
}
