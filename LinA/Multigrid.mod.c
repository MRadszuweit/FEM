...
#include "Multigrid.c.opari.inc"
#line 1 "Multigrid.c"


/* Bsp:
		amg_system_info* AMG_data = NULL;
		sparse_matrix* AMG_Bound = NULL;
		int** AMG_cond = NULL;

		set_var_number(1);
		sparse_matrix* I = set_matrix_Aij_00_2D(0,0,&insert_AV);
		sparse_matrix* AMG_sparse = set_matrix_Aij_11_2D(0,0,&insert_A_bbV);
		sparse_add(AMG_sparse,I,1.);
		int** AMG_cond = set_pure_conditions(1,DIRICHLET);
		AMG_Bound = FEM_get_dirichlet_correction(AMG_sparse,AMG_cond,1.);
		
		AMG_print_info(YES);
		AMG_set_SOR_relaxation_coeff(1.);
		AMG_data = AMG_setup(AMG_sparse,1,3,2,4,AGGRESSIVE,STAND_ALONE,STANDARD,NULL,NULL);
		
		free_sparse(AMG_sparse);
		free_sparse(I);
ende*/
...
extern int omp_chunk_size;  // aus linear_algebra.c

// globale Variablen

static double eps_minus = 0.3;
static double eps_plus = 0.5;
static double AMG_tol = 1.0e-1;

const int thread_number = 1;

static int AMG_max_iter = 20;
static double AMG_eps = 1e-8;
...
static void (*AMG_smoother)(sparse_matrix* A,double* F,double* Sol,int* Label,double alpha,int fix,int iter,int deg_freedom) = NULL;
...
void AMG_parallel_Jacobi_smoother(sparse_matrix* A,double* F,double* Sol,int* Label,double alpha,int fix,int iter,int deg_freedom){
	int i,j,k,l,I,ind,d,mesh_ind;							//parallel Jacobi method
	double sum,aii;
	int n = A->size;
	int N = n/deg_freedom;
	...
	for (k=0;k<iter;k++){		
		memcpy((void*)Old,(void*)Sol,n*sizeof(double));
#line 1372 "Multigrid.c"
		#pragma omp parallel private(i,j,d,I,sum,aii,ind)
		{	
			
#line 1375 "Multigrid.c"
...
void AMG_Jacobi_smoother(sparse_matrix* A,double* F,double* Sol,int* Label,double alpha,int fix,int iter){
	int i,j,k,ind,diag;							//Jacobi-Verfahren
	double sum;
	int n = A->size;
	double* X = clone_vector(Sol,n);
	for (k=0;k<iter;k++){		
		for (i=0;i<n;i++) if (Label==NULL || Label[i]!=fix){
			sum = F[i];
			diag = -1;
			for (j=0;j<A->Len[i];j++){
				ind = A->Indices[i][j];
				if (ind!=i) sum -= A->Values[i][j]*X[ind];
				else diag = j;
			}
			if (diag<0){
				printf("diagonal element is zero at index %d -> abort Jacobi\n",i);
				exit(0);
			}
			Sol[i] = sum/A->Values[i][diag];
		}
		copy_vector_to(Sol,X,n);
	}
	free(X);
}
...
double AMG_recursive_solution(amg_system_info* Data,double* F,double* Sol,int depth){
	...
	if (Data->AMG_coarsening==AGGRESSIVE && depth==Data->depth-1){
		...
		(*AMG_smoother)(Data->Coarse_A[depth],F,Sol,Data->Coarsening[depth],1.,UNDECIDED,1,AMG_deg_of_freedom);
	}
	else{
		smooth_iter = Data->AMG_smoothing_iterations;
		(*AMG_smoother)(Data->Coarse_A[depth],F,Sol,Data->Coarsening[depth],1.,UNDECIDED,smooth_iter,AMG_deg_of_freedom);
	}
	...
	for (k=0;k<Iter[depth];k++){
		...
		(*AMG_smoother)(Data->Coarse_A[depth],F,Sol,Data->Coarsening[depth],SOR_alpha,COARSE,smooth_iter,AMG_deg_of_freedom);
		//AMG_set_PCG_matrices(Data->Coarse_A[depth],Data->L[depth],Data->U[depth]);
		//pcg(Sol,F,Fine_sizes[depth],1E-8,1);
		(*AMG_smoother)(Data->Coarse_A[depth],F,Sol,Data->Coarsening[depth],SOR_alpha,UNDECIDED,smooth_iter,AMG_deg_of_freedom);
		// test residuum
...
void AMG_V_cycle(amg_system_info* Data,double* F,double* X,int level){
	int i,smooth_iter;
	//double r;
	//double r0 = ILU_residuum(Data->Coarse_A[level],Data->L[level],Data->U[level],X,F);
	if (AMG_smoother==NULL) AMG_smoother = AMG_SOR_smoother;
	int* Iter = (*(Data->Get_iter))(Data->depth);
	if (level==0) (*(Data->AMG_coarsest_solver))(Data->L[level],Data->U[level],F,X,Data->Fine_sizes[level]);
	else{		
		if (Data->AMG_coarsening==AGGRESSIVE && level==Data->depth-1) smooth_iter = Data->AMG_aggr_smoothing_iterations;
		else smooth_iter = Data->AMG_smoothing_iterations;	
		//if (AMG_info && level>0) printf("level %d cycles: %d\n",level,Iter[level]);
		for (i=0;i<Iter[level];i++){
			// presmoothing
			(*AMG_smoother)(Data->Coarse_A[level],F,X,Data->Coarsening[level],SOR_alpha,UNDECIDED,smooth_iter,AMG_deg_of_freedom);
			//AMG_smoother(Data->Coarse_A[level],F,X,Data->Coarsening[level],SOR_alpha,UNDECIDED,smooth_iter);
			// coarse residuum
			double* R = clone_vector(F,Data->Fine_sizes[level]);
			linear_map(R,-1.,Data->Coarse_A[level],X);
			double* Coarse_R = sparse_mult(Data->Restriction[level],R);
		
			// compute correction
			double* Correction = zero_vector(Data->Coarse_sizes[level]);
			AMG_V_cycle(Data,Coarse_R,Correction,level-1);
		
			// apply correction
			//s = scalar(R,R,Data->Fine_sizes[level])/sparse_bilinear(R,Data->Coarse_A[level],Correction);			
			linear_map(X,1.,Data->Prolongation[level],Correction);
	...
	/*if (level==Data->depth-1){
		r = ILU_residuum(Data->Coarse_A[level],Data->L[level],Data->U[level],X,F);
		if (AMG_info && level>0) printf("level: %d residuum: %e\n",level,r/r0);
	}
	else r = 0;*/
	free(Iter);
}

void FMG_recursive(amg_system_info* Data,double** F,double* X,int level){
	if (level==0) (*(Data->AMG_coarsest_solver))(Data->L[level],Data->U[level],F[level],X,Data->Fine_sizes[level]);
	else{
		double* Coarse_X = sparse_mult(Data->Restriction[level],X);		
		FMG_recursive(Data,F,Coarse_X,level-1);
		double* Fine_X = sparse_mult(Data->Prolongation[level],Coarse_X);
		AMG_V_cycle(Data,F[level],Fine_X,level);
		copy_vector_to(Fine_X,X,Data->Fine_sizes[level]);
		free(Coarse_X);
		free(Fine_X);
	}
}
...
double AMG_full_multigrid(amg_system_info* Data,double* F,double* Sol){
	...
	double** CF = (double**)malloc(max_depth*sizeof(double*));				// compute restricted F
	CF[max_depth-1] = clone_vector(F,Data->Fine_sizes[max_depth-1]);
	for (i=max_depth-1;i>0;i--){													
		CF[i-1] = sparse_mult(Data->Restriction[i],CF[i]);
	}
	
	//double r0 = ILU_residuum(Data->Coarse_A[max_depth-1],Data->L[max_depth-1],Data->U[max_depth-1],Sol,F);
	FMG_recursive(Data,CF,Sol,max_depth-1);
	double r = ILU_residuum(Data->Coarse_A[max_depth-1],Data->L[max_depth-1],Data->U[max_depth-1],Sol,F);
	
	for (i=0;i<max_depth;i++) free(CF[i]);
	free(CF);
	return r;
}
...
amg_system_info* AMG_setup(
	...
	//Info->Coarse_RA = (sparse_matrix**)malloc(depth*sizeof(sparse_matrix*));
	...
	/*if (VARIABLE_SYSTEM_MATRIX){
		Info->Pattern_RA = (product_pattern*)malloc(depth*sizeof(product_pattern));
		Info->Pattern_RAP = (product_pattern*)malloc(depth*sizeof(product_pattern));
	}
	else{
		Info->Pattern_RA = NULL;
		Info->Pattern_RAP = NULL;
	}*/
	...
	for (i=depth-1;i>0;i--){
		...
		printf("size %d : %d\n",i-1,Info->Coarse_sizes[i]);//AMG_get_coarse_number(Info->Coarsening[i],Info->Fine_sizes[i]));
		...
		// prepare fast products
		
		/*Info->Coarse_RA[i] = NULL;
		if (Info->Pattern_RAP!=NULL){
				Info->Pattern_RA[i] = set_product_pattern(Info->Restriction[i],Info->Coarse_A[i],NULL,&(Info->Coarse_RA[i]));
				Info->Pattern_RAP[i] = set_product_pattern(Info->Restriction[i],NULL,Info->Coarse_RA[i],NULL);		
						// hier kann optimiert werden: A.P und R.A.P wird hier zweimal berechnet				
		}*/
	...
	//Info->Coarse_RA[0] = NULL;
	return Info;
}

void AMG_recompute_coarse_matrices(amg_system_info* Info,sparse_matrix* New_matrix){
	if (Info->Coarse_A[Info->depth-1]!=NULL) {free_sparse(Info->Coarse_A[Info->depth-1]);}
	Info->Coarse_A[Info->depth-1] = clone(New_matrix);
	if (Info->Restriction!=NULL && Info->Prolongation!=NULL){
		int i;
		for(i=Info->depth-1;i>0;i--){
			if (Info->Coarse_A[i-1]!=NULL){
				free_sparse(Info->Coarse_A[i-1]);
				free_sparse(Info->L[i]);
				free_sparse(Info->U[i]);
			}
			Info->Coarse_A[i-1] = get_restricted_matrix(Info->Coarse_A[i],Info->Restriction[i],Info->Prolongation[i]);			
			Info->L[i] = sparse_zero(Info->Fine_sizes[i]);
			Info->U[i] = sparse_zero(Info->Fine_sizes[i]);
		...
		if (Info->L[0]!=NULL) {free_sparse(Info->L[0]);}
		if (Info->U[0]!=NULL) {free_sparse(Info->U[0]);}
		Info->L[0] = sparse_zero(Info->Fine_sizes[0]);
		Info->U[0] = sparse_zero(Info->Fine_sizes[0]);
		sparse_LU_factorization(Info->Coarse_A[0],Info->L[0],Info->U[0]);		
	}
...
/*void AMG_fast_coarse_matrices(amg_system_info* Info, sparse_matrix* M){
	int i,j,k,match;
	int n = M->size;
	sparse_matrix* A = Info->Coarse_A[Info->depth-1];
	for (i=0;i<n;i++){
		if (M->Len[i]==A->Len[i]){
			for (j=0;j<A->Len[i];j++) A->Values[i][j] = M->Values[i][j];
		}
		else{
			for (j=0;j<M->Len[i];j++){
				match = 0;
				for (k=0;k<A->Len[i];k++) if (M->Indices[i][j]==A->Indices[i][k]){
					A->Values[i][k] = M->Values[i][j];
					match = 1;
				}
				if (fabs(M->Values[i][j])>AMG_eps && match==0){
					printf("AMG_fast_coarse_matrices: sizes different in line %d -> abort\n",i);					
					exit(0);
	...
	for(i=Info->depth-1;i>0;i--){
		fast_sparse_product(Info->Coarse_RA[i],Info->Restriction[i],Info->Coarse_A[i],&(Info->Pattern_RA[i]));		// get B^T=(A.P)^T=P^T.A=R.A, when A=A^T, P^T=R
		fast_sparse_product(Info->Coarse_A[i-1],Info->Restriction[i],Info->Coarse_RA[i],&(Info->Pattern_RAP[i]));	// get R.B=R.A.P=R.(R.A)^T		
		
		if (Info->L[0]!=NULL) {free_sparse(Info->L[i]);}
		if (Info->U[0]!=NULL) {free_sparse(Info->U[i]);}
		Info->L[i] = sparse_zero(Info->Fine_sizes[i]);
		Info->U[i] = sparse_zero(Info->Fine_sizes[i]);
		incomplete_LU_factorization(Info->Coarse_A[i],Info->L[i],Info->U[i]);
	}
...
amg_statistics AMG_init_stat(const char* Out){
	amg_statistics Stat;
	Stat.calls = 0;
	Stat.Iterations = NULL;
	Stat.Residuals = NULL;
	Stat.max_iter = 0;
	Stat.max_res = 0;
	sprintf(Stat.File_name,"%s",Out);
	return Stat;
}
...
void AMG_add_stat(amg_statistics* Stat,int iter,double res){
	int i = Stat->calls; 
	(Stat->calls)++;
	Stat->Iterations = (int*)realloc(Stat->Iterations,Stat->calls*sizeof(int));
	Stat->Residuals = (double*)realloc(Stat->Residuals,Stat->calls*sizeof(double));
	Stat->Iterations[i] = iter;
	Stat->Residuals[i] = res;
	if (res>Stat->max_res) Stat->max_res = res;
	if (iter>Stat->max_iter) Stat->max_iter = iter;
}
...
void AMG_print_stat(amg_statistics* Stat){
	...
	int isum = 0;
	double dsum = 0;
	for (i=0;i<Stat->calls;i++){
		isum += Stat->Iterations[i];
		dsum += Stat->Residuals[i];
	}
	printf("average iterations: %f\n",(float)isum/Stat->calls);
	printf("average residual: %e\n",(float)dsum/Stat->calls);
	printf("maximum iterations: %d\n",Stat->max_iter);
	printf("maximum reidual: %e\n\n",Stat->max_res); 
}

int AMG_solve(amg_system_info* Data,double* F,double* Fine_Sol,amg_statistics* Stat){
	int local_iter;
	double res,res0,r,r0;
	double* Fine_F;
	...
	func_counter = 0;	
	r0 = matrix_residuum(A,Fine_Sol,Fine_F);
	res0 = ILU_residuum(Data->Coarse_A[d],Data->L[d],Data->U[d],Fine_Sol,Fine_F);
	r = r0;
	local_iter = 0;	
	...
	if (res0!=0){
		switch (Data->AMG_cycle_structure){
			case STAND_ALONE:														
				while(fabs(r/r0)>AMG_eps && local_iter<AMG_max_iter){					
					AMG_V_cycle(Data,Fine_F,Fine_Sol,d);					
					r = matrix_residuum(A,Fine_Sol,Fine_F);					
					local_iter++;
					//if (AMG_info) printf("sub residuum: %e\n",r/r0);
				}
				//if (AMG_info) printf("number of graph leafs: %d\n",func_counter/Iter[depth-1]);
				break;
			case FULL_AMG:				
				local_iter++;
				res = AMG_full_multigrid(Data,Fine_F,Fine_Sol);				
				break;
			default: printf("Warning AMG: no solution method choosen -> skip\n");
		}
		res = ILU_residuum(Data->Coarse_A[d],Data->L[d],Data->U[d],Fine_Sol,Fine_F);
		if (Stat!=NULL) AMG_add_stat(Stat,local_iter,fabs(r/r0));
		if (AMG_info==YES){
			printf("\niterations: %d\n",local_iter);
			printf("total relative residuum: %e\n",r/r0);
			printf("total relative ILU residuum: %e\n",res/res0);
		}
	}
	else{
		if (Stat!=NULL) AMG_add_stat(Stat,local_iter,0);
	}
	// clean
	free(Iter);
	free(Fine_F);
	//PCG_set_mat_mult(Old_matmult);
	//PCG_set_precon(Old_precon);
	return local_iter;
 }
...
void AMG_free_data(amg_system_info* Data){
	...
	for (i=0;i<Data->depth;i++){
		...
		//if (Data->Coarse_RA[i]!=NULL) {free_sparse(Data->Coarse_RA[i]);}
	}
	/*if (Data->Pattern_RA!=NULL){
		for (i=1;i<Data->depth;i++){
			free_product_pattern(&(Data->Pattern_RA[i]));
			free_product_pattern(&(Data->Pattern_RAP[i]));
		}
		free(Data->Pattern_RA);
		free(Data->Pattern_RAP);
	}*/			
