...
double* rank4_product(double* A,double* B,int dim,int mesh_size){
	...
	double* Res = zero_vector(dim*dim*dim*dim*mesh_size);
		...
		for (a=0;a<dim;a++){
			for (b=0;b<dim;b++){
				for (c=0;c<dim;c++){
					for (d=0;d<dim;d++){
						for (e=0;e<dim;e++){
							for (f=0;f<dim;f++) Res[mesh_size*(((a*dim+b)*dim+c)*dim+d)+i] += 
							A[mesh_size*(((a*dim+b)*dim+e)*dim+f)+i]*B[mesh_size*(((e*dim+f)*dim+c)*dim+d)+i];
						}
					}
				}
			}
...
double* rank4_tensor_product(double* A,double* B,double* C,double* D,int dim,int mesh_size){
	...
	double* Res = zero_vector(dim*dim*dim*dim*mesh_size);
	for (m=0;m<mesh_size;m++){
		for (i=0;i<dim;i++){
			for (j=0;j<dim;j++){
				for (k=0;k<dim;k++){
					for (l=0;l<dim;l++) Res[mesh_size*(((i*dim+j)*dim+k)*dim+l)+m] = A[mesh_size*i+m]*B[mesh_size*j+m]*C[mesh_size*k+m]*D[mesh_size*l+m];
				}
			}
		}
	}
...
void add_rank4_tensor_product(double* X,double* A,double* B,double* C,double* D,int dim,int mesh_size){
	double* Y = rank4_tensor_product(A,B,C,D,dim,mesh_size);
	vector_add(X,Y,1.,dim*dim*dim*dim*mesh_size);
	free(Y);
}
...
void sparse_multiplication(sparse_matrix* A,double* x){
	...
	for (i=0;i<n;i++){
		for (j=0;j<(A->Len[i]);j++){y[i] += A->Values[i][j]*x[A->Indices[i][j]];}
	}
...
long double hp_euklid_norm(double* x1,int n){
	long double s = hp_scalar(x1,x1,n);
	return sqrtl(s);
}
...
double* vector_join(double* x,double* y,int size_x,int size_y){
	int i;
	double* Res = (double*)malloc((size_x+size_y)*sizeof(double));
	for (i=0;i<size_x;i++) Res[i] = x[i];
	for (i=0;i<size_y;i++) Res[size_x+i] = y[i];
	return Res;
}
...
sparse_matrix* matrix_pseudo_left_mult(double* X,sparse_matrix* A,int dim){			// computes x_i*A_ij
	int i,j,k,n;
	if ((A->size % dim)!=0){
		printf("matrix_pseudo_mult: wrong dimension -> skip\n");
		return NULL;
	}
	else{
		n = A->size / dim;
		...
		for (i=0;i<Res->size;i++){
			k = i % n;
			for (j=0;j<Res->Len[i];j++) Res->Values[i][j] *= X[k];
		}
		return Res;
	}
}

sparse_matrix* matrix_pseudo_right_mult(double* X,sparse_matrix* A,int dim){			// computes A_ij*x_j
	int i,j,k,n;
	if ((A->size % dim)!=0){
		printf("matrix_pseudo_mult: wrong dimension -> skip\n");
		return NULL;
...
void sparse_pseudo_left_mult(double* X,sparse_matrix* A,int dim){			// computes x_i*A_ij
	int i,j,k,n;
	if ((A->size % dim)!=0){
		printf("sparse_pseudo_mult: wrong dimension -> skip\n");
	}
	else{
		n = A->size / dim;
		for (i=0;i<A->size;i++){
			k = i % n;
			for (j=0;j<A->Len[i];j++) A->Values[i][j] *= X[k];
		}
	}
}

void sparse_pseudo_right_mult(double* X,sparse_matrix* A,int dim){			// computes A_ij*x_j
	int i,j,k,n;
	if ((A->size % dim)!=0){
		printf("sparse_pseudo_mult: wrong dimension -> skip\n");
	}
...
//double sparse_pseudo_trilinear(double* z,double* y,sparse_matrix* A,double* x){  // computes (z_iy_iA_ijx_j
...
double quadratic_form(sparse_matrix* A,double* b,double* x){
	double res = sparse_bilinear(x,A,x)/2.;
	res -= scalar(b,x,A->size);
	return res;
}
...
void normalize_row_sum(sparse_matrix* A,double* Row_Sums){
	...
	for (i=0;i<A->size;i++){
		...
		if (Row_Sums==NULL) d = 1.; else d = Row_Sums[i];
		if (sum!=0) for (j=0;j<A->Len[i];j++) A->Values[i][j] *= d/sum;
	}
}

void normalize_col_sum(sparse_matrix* A,double* Col_Sums,int col_num){
	int i,j,k;
	double* Sums = zero_vector(col_num);
	for (i=0;i<A->size;i++){
		for (j=0;j<A->Len[i];j++){
			k = A->Indices[i][j];
			Sums[k] += A->Values[i][j];
	...
	for (i=0;i<A->size;i++){
		for (j=0;j<A->Len[i];j++){
			k = A->Indices[i][j];
			if (Sums[k]!=0) A->Values[i][j] *= Col_Sums[k]/Sums[k];			
		}
	}
	free(Sums);
}
...
double* sparse3D_Bvv(sparse_matrix3D* B,double* X1,double* X2){				// Babc.(X1b X2c)
	...
	for (i=0;i<n;i++){
		sum = 0;
		for(j=0;j<B->Len[i];j++){
			if (B->Indices1[i][j]>=n || B->Indices1[i][j]<0) printf("bad index1 at %d: %d\n",i,B->Indices1[i][j]);
...
double* sparse3D_vvB(sparse_matrix3D* B,double* X1,double* X2,int size){				// Babc.(X1a X2b)
	int i,j,k;
	int n = B->size;
	double* Res = zero_vector(size);
	for (i=0;i<n;i++){	
		for(j=0;j<B->Len[i];j++){
			//if (B->Indices2[i][j]>=size) printf("bad index2 at %d: %d\n",i,B->Indices2[i][j]);
			Res[B->Indices2[i][j]] += B->Values[i][j]*X1[i]*X2[B->Indices1[i][j]];	
		}		
	}
	return Res;
}
...
sparse_matrix3D* sparse3D_transpose13(sparse_matrix3D* B,int new_size){
	...
	sparse_matrix3D* Res = sparse_zero3D(new_size);
	for (i=0;i<B->size;i++){
		for (l=0;l<B->Len[i];l++){
			b = B->Values[i][l];
			if (b!=0){
				j = B->Indices1[i][l];
				k = B->Indices2[i][l];
				if (k>=new_size) printf("sparse3D_transpose13: wrong matrix size: (index %d)\n",k);
				len = (Res->Len[k])+1;
				
				Res->Len[k] = len;
				Res->Values[k] = (double*)realloc(Res->Values[k],len*sizeof(double));
				Res->Indices1[k] = (int*)realloc(Res->Indices1[k],len*sizeof(int));
				Res->Indices2[k] = (int*)realloc(Res->Indices2[k],len*sizeof(int));
				Res->Values[k][len-1] = b;
				Res->Indices1[k][len-1] = j;
				Res->Indices2[k][len-1] = i;				
			}			
		}
	}
	
	for (i=0;i<new_size;i++) if (Res->Len[i]==0){
		Res->Values[i] = zero_vector(1);
		Res->Indices1[i] = zero_int_list(1);
		Res->Indices2[i] = zero_int_list(1);
		Res->Len[i] = 1;
		Res->Indices1[i][0] = i;
		Res->Indices2[i][0] = i;
	}
...
int sparse3D_element_number(sparse_matrix3D* B){
	int i;
	int res = 0;
	for (i=0;i<B->size;i++) res += B->Len[i];
	return res;
}

void free_sparse_index_map(map_sparse** Map){
	free((*Map)->Indices);
	free_sparse((*Map)->Pattern);
	free(*Map);
	*Map = NULL;
}

void sparse3D_merge(sparse_matrix3D* B){
	int i,j,k,l,J1,J2,found;
	int elnum = sparse3D_element_number(B);
	for (i=0;i<B->size;i++) if (B->Len[i]>1){
		int len = 1;
		int* Newind1 = (int*)malloc(sizeof(int));
		int* Newind2 = (int*)malloc(sizeof(int));
		double* Newval = (double*)malloc(sizeof(double));
		Newind1[0] = B->Indices1[i][0];
		Newind2[0] = B->Indices2[i][0];
		Newval[0] = B->Values[i][0];
		for (j=1;j<B->Len[i];j++){
			J1 = B->Indices1[i][j];
			J2 = B->Indices2[i][j];
			found = 0;
			for (k=0;k<len;k++) if (Newind1[k]==J1 && Newind2[k]==J2){
				Newval[k] += B->Values[i][j];
				found = 1;
				break;
			}
			if (found==0){
				len++;
				Newind1 = (int*)realloc(Newind1,len*sizeof(int));
				Newind2 = (int*)realloc(Newind2,len*sizeof(int));
				Newval = (double*)realloc(Newval,len*sizeof(double));
				Newind1[len-1] = J1;
				Newind2[len-1] = J2;
				Newval[len-1] = B->Values[i][j];
			}					
		}
		free(B->Indices1[i]);
		free(B->Indices2[i]);
		free(B->Values[i]);
		B->Len[i] = len;
		B->Indices1[i] = Newind1;
		B->Indices2[i] = Newind2;
		B->Values[i] = Newval;
	}
	//printf("reduction: %f\n",(double)sparse3D_element_number(B)/elnum);
}
...
void print_sparse(sparse_matrix* A){
	...
	if (A!=NULL){
		...
		for (i=0;i<A->size;i++){
			...
			for (j=0;j<n;j++){
				sum += A->Values[i][j];
				fprintf(file,"(%d|%.15e)  ",A->Indices[i][j],A->Values[i][j]);
			}
	...
	if (A!=NULL){
		...
		for (i=0;i<A->size;i++){
			...
			for (j=0;j<A->Len[i];j++) if (A->Values[i][j]!=0){
				counter++;
				flag = 1;
				if (A->Indices[i][j]+1>cols) cols = A->Indices[i][j]+1;
			}
			if (flag && i+1>rows) rows = i+1;
		}
		...
		for (i=0;i<A->size;i++){
			for (j=0;j<A->Len[i];j++){
				a = A->Values[i][j];
				if (a!=0) fprintf(file,"%d %d %g\n",i+1,A->Indices[i][j]+1,a);
			}
		}	
		fclose(file);
	}
...
void add_subsparse_to_sparse(sparse_matrix* A,sparse_matrix* B,int i_o,int j_o){
	...
	for (i=0;i<B->size;i++){
		for (k=0;k<B->Len[i];k++){
			j = B->Indices[i][k];
			b = B->Values[i][k];
			if (b!=0) insert_sparse(A,b,i+i_o,j+j_o);			
		}
	}
}
...
sparse_matrix3D* get_transpose3D_12(sparse_matrix3D* B,int new_size){			// Bijk -> Bjik
	int i,j,k,l;
	...
	for (i=0;i<n;i++){
		for (l=0;l<B->Len[i];l++){
			...
			if (j>=new_size){
				printf("get_transpose3D_12: wrong matrix size -> abort\n");
				exit(0);
			}
			(Res->Len[j])++;
			Res->Indices1[j] = (int*)realloc(Res->Indices1[j],(Res->Len[j])*sizeof(int));
			Res->Indices2[j] = (int*)realloc(Res->Indices2[j],(Res->Len[j])*sizeof(int));
			Res->Values[j] = (double*)realloc(Res->Values[j],(Res->Len[j])*sizeof(double));
			
			Res->Indices1[j][Res->Len[j]-1] = i;
			Res->Indices2[j][Res->Len[j]-1] = k;
			Res->Values[j][Res->Len[j]-1] = B->Values[i][l];
		}		
	}
	sparse3D_merge(Res);
...
sparse_matrix3D* get_transpose3D_13(sparse_matrix3D* B,int new_size){			// Bijk -> Bkji
	int i,j,k,l;
	...
	for (i=0;i<n;i++){
		for (l=0;l<B->Len[i];l++){
			...
			if (k>=new_size){
				printf("get_transpose3D_13: wrong matrix size -> abort\n");
				exit(0);
			}
			(Res->Len[k])++;
			Res->Indices1[k] = (int*)realloc(Res->Indices1[k],(Res->Len[k])*sizeof(int));
			Res->Indices2[k] = (int*)realloc(Res->Indices2[k],(Res->Len[k])*sizeof(int));
			Res->Values[k] = (double*)realloc(Res->Values[k],(Res->Len[k])*sizeof(double));
			
			Res->Indices1[k][Res->Len[k]-1] = j;
			Res->Indices2[k][Res->Len[k]-1] = i;
			Res->Values[k][Res->Len[k]-1] = B->Values[i][l];
		}		
	}
	sparse3D_merge(Res);
...
/*void sorted_list_insert(int** List,int* len,int index){
	int pos;
	int flag = -1;
	if (*List==NULL){
		*List = (int*)malloc(sizeof(int));
		...
		pos = get_position(*List,*len,index,&flag);
		if (flag>=0)
	}
}*/
...
int find_position_in_row(sparse_matrix* A,int row,int col){
	int flag = -1;
	int pos = get_position(A->Indices[row],A->Len[row],col,&flag);
	if (flag==-1){
		printf("error: index (%d,%d) does not exist-> abort\n",row,col);
		exit(0);
	}
	return pos;
}
...
void double_sort(int* List,double* Values,int size){  // divide & conquer
	if (size>1){									  // List wird permutiert
		...
		int* Bigger = (int*)malloc(size*sizeof(int));
...
void divide_and_conquer_sort(int* List,void** Values,int size,int (*Compare)(void* X1,void* X2)){	// divide & conquer
	if (size>1){			  				   													    // List wird permutiert
		int i;																					    // Values bleibt unverändert
		int s_size = 0;
		int b_size = 0;
		...
		int* Bigger = (int*)malloc(size*sizeof(int));
		for (i=1;i<size;i++){						
			if ((*Compare)(Values[List[i]],Values[a])>0){  //if (Values[List[i]]>Values[a]){
				b_size++;			
				Bigger[b_size-1] = List[i];
			}
			else{
		...
		if (1+b_size+s_size != size) printf("Fehler bei dc sort\n");
		Bigger = (int*)realloc(Bigger,b_size*sizeof(int));
		Smaller = (int*)realloc(Smaller,s_size*sizeof(int));
		divide_and_conquer_sort(Smaller,Values,s_size,Compare);
		divide_and_conquer_sort(Bigger,Values,b_size,Compare);
		for (i=0;i<s_size;i++) List[i] = Smaller[i];
		List[s_size] = a;
		for (i=0;i<b_size;i++) List[s_size+i+1] = Bigger[i];
		free(Smaller);
...
void apply_permutation(void* List,int element_number,size_t element_size,int* Permutation){
	int i;
	size_t pos;
	void* Temp = (void*)malloc(element_number*element_size);
	
	for (i=0;i<element_number;i++){
		pos = Permutation[i]*element_size;
		memcpy(Temp+i*element_size,List+pos,element_size);
	}
	memcpy(List,Temp,element_number*element_size);
	free(Temp);
}
...
void int_sort(int* List,int size){
	...
	int* Map = get_ID_index_map(size);
	
	divide_and_conquer_sort(Map,(void**)Temp,size,&cmp);
	apply_permutation((void*)List,size,sizeof(int),Map);
	free(Map);
	free(Temp);
}
...
void join_lists(int** List,int* size,int* L1,int* L2,int s1,int s2){
	...
	int i,p,val;
	int t = s1+s2;
	int* Perm  = zero_int_list(t);
	int** Temp = (int**)malloc(t*sizeof(int*));
	for (i=0;i<t;i++){
		if (i<s1) Temp[i] = &(L1[i]); else Temp[i] = &(L2[i-s1]);
		Perm[i] = i;
	}
		
	divide_and_conquer_sort(Perm,(void**)Temp,t,&cmp);
...
int cmp_multidim_index(void* X1,void* X2){
	...
	if (X1==NULL){
		...
		int i;
		matrix_element* E1 = (matrix_element*)X1;
		matrix_element* E2 = (matrix_element*)X2;
		int res = 0;
		for (i=0;i<dim;i++){
			if (E1->Ind[i]!=E2->Ind[i]){
				if (E1->Ind[i]>E2->Ind[i]) res = 1; else res = -1;
				break;
			}
		}
		return res;
...
void sparse_general_sort(sparse_general** C){
	int i,I,new_size;
	double sum;
	matrix_element* E;
	void* E_prev;
	...
	sparse_general* A = *C;
	int old_size = A->size;
	cmp_multidim_index(INIT,&(A->rank));
	int* Map = get_ID_index_map(old_size);	
	void** Values = (void**)malloc(old_size*sizeof(void*));
	for (i=0;i<old_size;i++) Values[i] = &(A->Elements[i]);
	divide_and_conquer_sort(Map,Values,old_size,&cmp_multidim_index);
	
	sum = A->Elements[Map[0]].val;
	E_prev = Values[Map[0]];
	sparse_general* Res = new_sparse_general(A->rank,A->Dimension);
	for (i=1;i<old_size;i++){
		E_curr = Values[Map[i]];
		if (cmp_multidim_index(E_prev,E_curr)==0) sum += A->Elements[Map[i]].val;
		else{
			E = (matrix_element*)E_prev;
			sparse_general_Append(Res,sum,A->rank,E->Ind);
			E = (matrix_element*)E_curr;
			sum = E->val;
			E_prev = E_curr;		
		}
	}
	...
	/*sum = 0;
	for (i=0;i<A->size;i++) sum +=A->Elements[i].val;
	for (i=0;i<Res->size;i++) sum -=Res->Elements[i].val;
	printf("check diff %e\n",sum);*/

	free(Values);
	free(Map);
	free_sparse_general(C);
	*C = Res;
}

int divide_and_conquer_get_position(void* Element,void** Values,int l,int r,int* Flag,int (*Compare)(void* X1,void* X2)){
	int pos = (l+r)/2;
	if (pos == l){
		*Flag = 0;
		return pos+1;
		...
		if ((*Compare)(Values[pos],Element)<0) return divide_and_conquer_get_position(Element,Values,pos,r,Flag,Compare);
		if ((*Compare)(Values[pos],Element)>0) return divide_and_conquer_get_position(Element,Values,l,pos,Flag,Compare);
		if ((*Compare)(Values[pos],Element)==0){
			*Flag = 1;
			return pos;
		}
	}
...
int DC_get_position(void* Element,void** Values,int size,int* Flag,int (*Compare)(void* X1,void* X2)){  // list muss sortiert sein !
	*Flag = 0;
	if (size>0){
		if ((*Compare)(Values[0],Element)<0 && (*Compare)(Values[size-1],Element)>0) return divide_and_conquer_get_position(Element,Values,0,size-1,Flag,Compare);
		else{
			if ((*Compare)(Values[0],Element)>=0){
				if ((*Compare)(Values[0],Element)==0) *Flag = 1; else *Flag = 0;
				return 0;
			}
			if ((*Compare)(Values[size-1],Element)<=0){
				if ((*Compare)(Values[size-1],Element)==0){
					*Flag = 1;
					return size-1;
				}
...
void remove_zero(sparse_matrix* A,int i){
	int j;
	int k = -1;
	for (j=0;j<A->Len[i];j++) if (A->Values[i][j]==0) k=j;
	if (k>=0) remove_element_at(A,i,k);
}

void remove_zeros(sparse_matrix* A){
	int i;
	for (i=0;i<A->size;i++) remove_zero(A,i);
}

void apply_func(double* X,int size,double (*Func)(double x)){
	int i;
	for (i=0;i<size;i++) X[i] = (*Func)(X[i]);
}
...
void SOR(sparse_matrix* A,double* F,double* Sol,double alpha,int iter){
	...
	if (iter>0){
		...
		do{
			...
			res = matrix_residuum(A,Sol,F);
			printf("residuum: %e\n",res);
			k++;
		}while(fabs(res)>eps && k<max_iter);
		if (k==max_iter) printf("Warning: SOR did not converge after %d iterations\n",max_iter);	
	}
}
...
double* get_col_sums(sparse_matrix* A,int col_num){
	int i,j;
	double* Res = zero_vector(col_num);
	for (i=0;i<A->size;i++){
		for (j=0;j<A->Len[i];j++) Res[A->Indices[i][j]] += A->Values[i][j];
	}
	return Res;
}
...
void sub_sparse_insert(sparse_matrix* A,sparse_matrix* Small,int equ,int var,int d){
	...
	for (i=0;i<Small->size;i++){
		for (j=0;j<Small->Len[i];j++) insert_sparse(A,Small->Values[i][j],i+equ*d,(Small->Indices[i][j])+var*d);
	}
}
...
sparse_matrix* get_sparse_part(sparse_matrix* A,int start,int end){
	...
	sparse_matrix* Res = (sparse_matrix*)malloc(sizeof(sparse_matrix));
	Res->size = end-start;
	Res->Len = zero_int_list(end-start);
	Res->Indices = (int**)malloc((end-start)*sizeof(int*));
	Res->Values = (double**)malloc((end-start)*sizeof(double*));
	
	for (i=start;i<end;i++){
		Res->Len[i-start] = A->Len[i];
		Res->Indices[i-start] = clone_list(A->Indices[i],A->Len[i]);
		Res->Values[i-start] = clone_vector(A->Values[i],A->Len[i]);
		for (j=0;j<Res->Len[i-start];j++) if (Res->Values[i-start][j]==0 && Res->Indices[i-start][j]==i) Res->Indices[i-start][j] = i-start;
	}
...
sparse_matrix* add_constraints_default(sparse_matrix* A,sparse_matrix* Constraints,int* ActiveSet,int active_size){
	int i,j,k;
	int N = A->size;
	int n = Constraints->size;
	sparse_matrix* Res = clone(A);
	sparse_matrix* CT = get_transpose(Constraints,n);
	enlarge_matrix(Res,N,N+active_size,0,0);
	
	for (k=0;k<active_size;k++){
		i = ActiveSet[k];
		for (j=0;j<CT->Len[i];j++){			
			insert_sparse(Res,CT->Values[i][j],CT->Indices[i][j],N+k);
			insert_sparse(Res,CT->Values[i][j],N+k,CT->Indices[i][j]);
		}		
	}
	free_sparse(CT);
	return Res;
...
sparse_matrix* add_constraints_flux(sparse_matrix* A,sparse_matrix* Constraints,int* ActiveSet,int active_size){
	int i,j,k;
	int N = A->size;
	int n = Constraints->size;
	sparse_matrix* Res = clone(A);
	//enlarge_matrix(Res,N,N+active_size,0,0);
	simple_enlarge_matrix(Res,active_size);
	
	for (k=0;k<active_size;k++){
		i = ActiveSet[k];
		for (j=0;j<Constraints->Len[i];j++){
			// Warning: adds constraints to second equation !
			// Only for diagonal constraints
			insert_sparse(Res,Constraints->Values[i][j],n+Constraints->Indices[i][j],N+k);
			insert_sparse(Res,Constraints->Values[i][j],N+k,Constraints->Indices[i][j]);
		}		
	}
	
	/*if (active_size>0){
		double* V = generate_vector(Res->size,1.);
		double* K = zero_vector(Res->size);
		for (k=0;k<active_size;k++){
			K[n+ActiveSet[k]] = 1.;
			K[N+k] = 1.;
		}
		sparse_multiplication(Res,K);
		printf("\nkernel res: %e:%e\n",euklid_norm(K,Res->size),euklid_norm(V,Res->size));
		free(V);
		free(K);
	}*/
...
void free_ass_info(ass_info** info){
	if (*info!=NULL){
		if ((*info)->Active!=NULL) free((*info)->Active);
		if ((*info)->Signs!=NULL) free((*info)->Signs);
		if ((*info)->Constraint!=NULL) free((*info)->Constraint);
		if ((*info)->Iter_history!=NULL) free((*info)->Iter_history);
		free(*info);
	}
}

// positive definite: look for minimum
// negative definite: look for maximum

int active_set_solver(sparse_matrix* A,double* Sol,double* b,sparse_matrix* B,double* L,double* U
 ,int (*Solver)(sparse_matrix* A,double* X,double* b),int max_iter,double definiteness,int mode,ass_info** solver_info){
	const double tol = 1e-10;
	
	/*static int active_size = 0;
	static int* Active = NULL;
	static double* Signs = NULL;
	static double* Constraint = NULL;*/
	
	if (*solver_info==NULL){
		*solver_info = (ass_info*)malloc(sizeof(ass_info));
		(*solver_info)->Active = NULL;
		(*solver_info)->Signs = NULL;
		(*solver_info)->Constraint = NULL;
		(*solver_info)->Iter_history = NULL;
		(*solver_info)->active_size = 0;
		(*solver_info)->calls = 0;
	}
	...
	/*if (A==NULL){
		...
		if (Constraint!=NULL) free(Constraint);
	...
	int i,j,k,iter,fulfilled;
	sparse_matrix* M;

	int n = A->size;
	int m = B->size;	
	
	if ((*solver_info)->Active==NULL) (*solver_info)->Active = zero_int_list(m);
	if ((*solver_info)->Signs==NULL) (*solver_info)->Signs = zero_vector(m);
	if ((*solver_info)->Constraint==NULL) (*solver_info)->Constraint = zero_vector(m);
	...
	do{
		...
		int* ActiveSet = (int*)malloc(((*solver_info)->active_size)*sizeof(int));
		double* ActiveSigns = (double*)malloc(((*solver_info)->active_size)*sizeof(double));
		Z = realloc(Z,(n+((*solver_info)->active_size))*sizeof(double));
		k = 0;
		for (i=0;i<m;i++) if ((*solver_info)->Active[i]){			
			ActiveSet[k] = i;
			ActiveSigns[k] = (*solver_info)->Signs[i];
			Z[n+k] = (*solver_info)->Constraint[i];
			k++;
		}
		if (k!=((*solver_info)->active_size)) printf("\nsomenthing's wrong in active_set_solver\n");
		
		// solve linear system with constraints
		if (mode==ASS_FLUX) M = add_constraints_flux(A,B,ActiveSet,((*solver_info)->active_size));	
		else M = add_constraints_default(A,B,ActiveSet,((*solver_info)->active_size));	
		(*Solver)(M,&(X[0]),&(Z[0]));
		free_sparse(M);
		
		fulfilled = 1;
		// test sign of KKT multipliers -> delete constraints if not feasible
		double* Y = sparse_mult(B,&(X[0]));
		for (k=0;k<((*solver_info)->active_size);k++){
			if (ActiveSigns[k]*X[n+k]*definiteness>0){
				(*solver_info)->Active[ActiveSet[k]] = 0;
				((*solver_info)->active_size)--;
				fulfilled = 0;
			}
		}
		...
		for (i=0;i<m;i++){
			if (Y[i]<L[i]-tol || Y[i]>U[i]+tol){		
				if (!(*solver_info)->Active[i]) ((*solver_info)->active_size)++;			
				(*solver_info)->Active[i] = 1;
				if (Y[i]<L[i]-tol){
					(*solver_info)->Signs[i] = 1.;
					(*solver_info)->Constraint[i] = L[i];
				}
				if (Y[i]>U[i]+tol){
					(*solver_info)->Signs[i] = -1.;
					(*solver_info)->Constraint[i] = U[i];
				}
				fulfilled = 0;
			}
		}
		...
		free(ActiveSigns);
		iter++;		
	}while(!fulfilled && iter<max_iter);
	
	copy_vector_content(X,Sol,0,0,n);
	if (iter>=max_iter) printf("Warning: no convergence in %d iterations\n",iter);
	
	(*solver_info)->Iter_history = (int*)realloc((*solver_info)->Iter_history,((*solver_info)->calls+1)*sizeof(int));
	(*solver_info)->Iter_history[(*solver_info)->calls] = iter;
	((*solver_info)->calls)++;
...
/*void clean_active_solver(){
	active_set_solver(NULL,NULL,NULL,NULL,NULL,NULL,NULL,0,0,0);
}*/
...
/*sparse_general* sparse_general_contraction(sparse_general* C,double** X,int* Contr_index,int num_fields){
	...
	double c;
	int* Indices;
	sparse_general* Res = (sparse_general*)malloc(sizeof(sparse_general));
	Res->Values = clone_vector(C->Values,C->size);
	Res->Indices = (int**)malloc(C->size*sizeof(int*));
	Res->size = C->size;
	
	new_size = 0;
	int* New_pos = (int*)malloc(C->dimension*sizeof(int));
	for (k=0;k<C->dimension;k++){
		if (in_list(Contr_index,num_fields,k)<0){
			New_pos[k] = new_size;
			new_size++;
		}
		else New_pos[k] = -1;
	}
	Res->dimension = new_size;
	
	for (i=0;i<Res->size;i++){
		for (k=0;k<num_fields;k++) Res->Values[i] *= X[Contr_index[k]][C->Indices[i][Contr_index[k]]];
		Indices = zero_int_list(new_size);
		for (k=0;k<C->dimension;k++) if (New_pos[k]>=0) Indices[New_pos[k]] = C->Indices[i][k];
		Res->Indices[i] = Indices;
	}
	return Res;
}
...
double* sparse_general_to_vector(sparse_general* C,double vec_size){
	int i;
	double* Res = zero_vector(vec_size);
	for (i=0;i<C->size;i++) Res[C->Indices[i][0]] += C->Values[i];
	return Res;
}

sparse_matrix* sparse_general_to_matrix(sparse_general* C,int col_size){
	int i,j,k;
	sparse_matrix* Res = sparse_zero(col_size);
	for (k=0;k<C->size;k++){
		i = C->Indices[k][0];
		j = C->Indices[k][1];
		insert_sparse(Res,C->Values[k],i,j);
	}
	return Res;
}*/
...
sparse_general* new_sparse_general(int rank,int* Dim){
	...
	Res->size = 0;
	Res->rank = rank;
	Res->Dimension = clone_list(Dim,rank);
	return Res;
}

void free_sparse_general(sparse_general** A){
	int i;
	for (i=0;i<(*A)->size;i++) free((*A)->Elements[i].Ind);
	free((*A)->Elements);
	free((*A)->Dimension);
	free(*A);
}
...
void sparse_general_append(sparse_general* A,double val,int rank,...){
	if (A->rank==rank){
		...
		va_list arg_list;
		A->size++;
		A->Elements = (matrix_element*)realloc(A->Elements,A->size*sizeof(matrix_element));
		A->Elements[A->size-1].val = val;
		A->Elements[A->size-1].Ind = zero_int_list(rank);
		va_start(arg_list,rank);
		for (i=0;i<rank;i++) A->Elements[A->size-1].Ind[i] = va_arg(arg_list,int);
	}
	else{
		printf("sparse general matrix append: Wrong element rank -> abort");
		exit(0);
	}
}
...
void sparse_general_Append(sparse_general* A,double val,int rank,int* Ind){
	if (A->rank==rank){
		...
		A->Elements = (matrix_element*)realloc(A->Elements,A->size*sizeof(matrix_element));
		A->Elements[A->size-1].val = val;
		A->Elements[A->size-1].Ind = clone_list(Ind,rank);
	}
	else{
		printf("sparse general matrix append: Wrong element rank -> abort");
		exit(0);
...
void sparse_general_add(sparse_general* A,sparse_general* B,double factor,int sorted){		// A sorted -> 1 
	int i,j,pos,new_size;																	// not	    -> 0
	int found = 0;
	if (A->rank!=B->rank){
		printf("addition not possible: different rank -> abort\n");
		exit(0);
	}
	int n = A->size;
	if (sorted){		
		A->Elements = (matrix_element*)realloc(A->Elements,(A->size+B->size)*sizeof(matrix_element));
		matrix_element* NotFound = NULL;
		new_size = n;
		void** Indices = (void**)malloc(n*sizeof(void*));
		for (i=0;i<n;i++) Indices[i] = &(A->Elements[i]);
		for (i=0;i<B->size;i++){			
			pos = DC_get_position(&(B->Elements[i]),Indices,n,&found,&cmp_multidim_index);
			if (found) A->Elements[pos].val += factor*B->Elements[i].val;
			else{
				new_size++;			
				A->Elements[new_size-1].val = factor*B->Elements[i].val;
				A->Elements[new_size-1].Ind = clone_list(B->Elements[i].Ind,B->rank);
			}
		}
		A->Elements = (matrix_element*)realloc(A->Elements,new_size*sizeof(matrix_element));
		free(Indices);
	}
	else{
		A->Elements = (matrix_element*)realloc(A->Elements,(A->size+B->size)*sizeof(matrix_element));
		A->size += B->size;
		for (i=0;i<B->size;i++){
			A->Elements[n+i].val = factor*B->Elements[i].val;
			A->Elements[n+i].Ind = clone_list(B->Elements[i].Ind,B->rank);		
		}
	}
}
...
void sparse_general_scalar_mult(sparse_general* A,double factor){
	int i;
	for (i=0;i<A->size;i++) A->Elements[i].val *= factor;
}

sparse_general* sparse_general_contraction(sparse_general* A,int arg_num,...){
	int i,j,k,I,J,new_rank;
	double a;
	sparse_general* Res = NULL;
	double** X = NULL;
	if (arg_num==A->rank){
		int* Map = zero_int_list(A->rank);
		X = (double**)malloc(arg_num*sizeof(double*));
		va_list arg_list;
		...
		va_end(arg_list);
		Res = (sparse_general*)malloc(sizeof(sparse_general));
		Res->Elements = (matrix_element*)malloc(A->size*sizeof(matrix_element));
		Res->size = A->size;
		Res->rank = k;
		Res->Dimension = zero_int_list(k);
		for (j=0;j<A->rank;j++) if (X[j]!=NULL && Map[j]>=0) Res->Dimension[Map[j]] = A->Dimension[j];
		for (i=0;i<A->size;i++){	
			Res->Elements[i].val = A->Elements[i].val;
			Res->Elements[i].Ind = zero_int_list(k);		
			for (j=0;j<A->rank;j++){
				if (X[j]!=NULL){
					if (A->Elements[i].Ind[j]<A->Dimension[j]) Res->Elements[i].val *= X[j][A->Elements[i].Ind[j]];
					else{
						printf("error in matrix contraction: index exceeds dimension at %d\n",i);
						exit(0);
					}
					
				}
				else if (Map[j]>=0) Res->Elements[i].Ind[Map[j]] = A->Elements[i].Ind[j];
			}						
		}
		free(Map);
		free(X);
...
sparse_matrix* sparse_general_to_sparse2D(sparse_general* A,int arg_num,...){
	...
	if (arg_num>0){
		...
		va_end(arg_list);
		Res = sparse_zero(A->Dimension[k]);
		for (i=0;i<A->size;i++){
			I = -1;
			factor = 1.;
			for (j=0;j<A->rank;j++){
				k = A->Elements[i].Ind[j];
				if (X[j]==NULL){
					if (I==-1) I = k; else J = k;
				}
				else factor *= X[j][k];
			}
			insert_sparse(Res,factor*A->Elements[i].val,I,J);
		}
		free(X);		
    }
    else if (arg_num==0 && A->rank==2){
		Res = sparse_zero(A->Dimension[0]);
		for (i=0;i<A->size;i++) insert_sparse(Res,A->Elements[i].val,A->Elements[i].Ind[0],A->Elements[i].Ind[1]);			
	}
	return Res;
}
...
double* sparse_general_to_vector(sparse_general* A,int arg_num,...){
	...
	if (arg_num>0){
		...
		for (i=0;i<A->size;i++){
			...
			if (I>=n || I<0){
				printf("error in sparse_general_to_vector: index %d exceeds dimension ->abort\n",I);
				exit(0);
			}
			else Res[I] += factor*A->Elements[i].val;
		}
		free(X);		
    }
    else if (arg_num==0 && A->rank==1){
		Res = zero_vector(A->Dimension[0]);
		for (i=0;i<A->size;i++) Res[A->Elements[i].Ind[0]] += A->Elements[i].val;			
	}
	return Res;
}
...
double sparse_general_to_scalar(sparse_general* A,int arg_num,...){
	...
	if (arg_num>A->rank){
		...
		va_list arg_list;
		va_start(arg_list,arg_num);
		for (i=0;i<arg_num;i++) X[i] = va_arg(arg_list,double*);
		va_end(arg_list);
		for (i=0;i<A->size;i++){
			factor = 1.;
			for (j=0;j<A->rank;j++){
...
sparse_general* sparse3D_to_general(sparse_matrix3D* B,int dim1,int dim2,int dim3){
	...
	for (i=0;i<B->size;i++){
		for (j=0;j<B->Len[i];j++){
			sparse_general_append(Res,B->Values[i][j],rank,i,B->Indices1[i][j],B->Indices2[i][j]);
		}
	}
	sparse_general_sort(&Res);
	return Res;
}
...
double general_matrix_norm(sparse_general* A){
	int i;
	double res = 0;
	for (i=0;i<A->size;i++) res += fabs(A->Elements[i].val);
	return (double)res/A->size;
}

void print_sparse_general(sparse_general* A){
	const char* Name = "/Home/damage/radszuwe/Daten/general";
	int i,j;
	char Line[512];
	char Ele[128];
	FILE* file = fopen(Name,"w");
	fprintf(file,"rank %d\n",A->rank);
	sprintf(Line,"dimensions:\t");
	for (i=0;i<A->rank;i++){
		sprintf(Ele,"%d\t",A->Dimension[i]);
		strcat(Line,Ele);
	}
	fprintf(file,"%s\n",Line);
	
	for (i=0;i<A->size;i++){
		sprintf(Line,"%e\t",A->Elements[i].val);
		for (j=0;j<A->rank;j++){
			sprintf(Ele,"%d\t",A->Elements[i].Ind[j]);
			strcat(Line,Ele);
		}
		fprintf(file,"%s\n",Line); 
	}
