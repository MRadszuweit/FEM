...
double f_c1(double x,double a){
	double Na = a*x;
	double d = 1.+Na*Na;
	return NM*(-2.*a*Na*Na*(1.+2.*Na)/(d*d)+a*(1.+4.*Na)/d);
}

double f_c2(double x,double a,double b){
	return NM*((x*(-a+b-2*(a*a-b*b)*x+a*(a-b)*b*x*x))/((1.+a*a*x*x)*(1.+b*b*x*x)));
}
...
double Smith4_F(double* X,int i,double t,int var_index){
	switch(var_index){
		...
		case 1: return 0;
		case 2: return 0;
		case 3: return saturated_fT(X,i);*/
	}
	return 0;
}
...
double test_FWD_Euler_Smith(double* Sol,double* Old_Sol){
	int i,j;
	double sum;
	double* X = zero_vector(4);
	double* B = zero_vector(4);
	double* R = zero_vector(4);
	...
	for (i=0;i<n;i++){
		...
		B[1] = 0;
		B[2] = Old_Sol[2*n+i];
		B[3] = Old_Sol[i]+Old_Sol[n+i];
		for (j=0;j<4;j++){
			R[j] = FWD_Euler_Smith(X,j,4)-B[j];
			sum += R[j]*R[j];
			if (fabs(R[j])>0.1) printf("voll scheisen bei index %d|%d\n",i,j); 
		}
	}
	free(X);
	free(B);
...
void RDA_part(double* Solution,double* Old_Solution,double t,double dt){
	...
	//operator splitting
		...
		//Runge_Kutta4(Sol,N*n_mesh,N,t,dt,5,&Hopf_F);
		...
		set_system_matrix(E);
		r0 = matrix_residuum(E,CV,CE);
		if (r0>0) FEM_solver(CE,CV,&matvec,L_Adv,U_Adv);
		free_sparse(E);
		free_sparse(U_Adv);
		free_sparse(L_Adv);
		...
		copy_vector_content(CV,Sol,0,0,n_mesh);					//Konvektion an 
		free_sparse(Adv);
		free(CV);
		free(CE);
		free(C);
		...
		/*int n_xy = (int)round((double)sqrt(interpolation_size));
		double* C = restrict_vector_by(n_c*n_mesh,(n_c+1)*n_mesh,Sol);
		
		double max = get_max_element_abs(C,n_mesh);		// Achtung: test !
		if (max>10000.0){
			printf("too large values -> abort\n");
			exit(0);
		}
		if(test_negative(C,n_mesh)){
			printf("warning: negative concentration\n");
			remove_negative(C,n_mesh);
			//printf("negative concentration -> abort\n");
			//exit(0);
		}
		
		double* Vx = restrict_vector_by(n_v*n_mesh,(n_v+1)*n_mesh,Solution);
		double* Vy = restrict_vector_by((n_v+1)*n_mesh,(n_v+2)*n_mesh,Solution);
		//double* Vx = get_function(NULL,n_mesh,1,0,&test_n);
		//double* Vy = zero_vector(n_mesh);
		double* Ux = restrict_vector_by(n_u*n_mesh,(n_u+1)*n_mesh,Solution);
		double* Uy = restrict_vector_by((n_u+1)*n_mesh,(n_u+2)*n_mesh,Solution);
		vector_add(Vx,Ux,-1.,n_mesh); 
		vector_add(Vy,Uy,-1.,n_mesh);
		free(Ux);
		free(Uy);
		
		double* CI = zero_vector(interpolation_size);
		double* VIx = zero_vector(interpolation_size);
		double* VIy = zero_vector(interpolation_size);
		double tot_C = get_total_amount(C,0);
		mesh_interpolation(Interpolation_mesh,CI,interpolation_size,C,Interpolation_Map);
		mesh_interpolation(Interpolation_mesh,VIx,interpolation_size,Vx,Interpolation_Map);
		mesh_interpolation(Interpolation_mesh,VIy,interpolation_size,Vy,Interpolation_Map);
		point2D* Moved_mesh = regular_move2D(Interpolation_mesh,n_xy,n_xy,i_mesh_dh.x,i_mesh_dh.y,dt,VIx,VIy);
		apply_regular_move2D(Interpolation_mesh,Moved_mesh,n_xy,n_xy,i_mesh_dh.x,i_mesh_dh.y,CI);
		inverse_interpolation(Interpolation_mesh,CI,C,Inv_Interpolation_Map);
		double new_C = get_total_amount(C,0);
		printf("tcd: %f\n",new_C/tot_C);
		if (abs(tot_C/new_C-1.)>1E-4) printf("Warning: large deviation after convection step: %e\n",abs(tot_C/new_C-1.));
		//scalar_mult(tot_C/new_C,C,n_mesh);
		copy_vector_content(C,Sol,0,0,n_mesh);					//Konvektion an
		...
		test_vector(Sol,4*n_mesh,"bad pos 1");*/
		
		
		// global coupling
		
		/*double* dT = get_function(Sol,n_mesh,N,n_c+3,&slow_fT);
		copy_vector_content(Sol,dT,0,0,3*n_mesh);
		sparse_matrix* B1 = set_matrix_Bijk_000(n_c,n_c,dT,n_c+3,&insert_AV);		
		double* Glob = global_quadratic_average_coupling(dT,n_c,n_c,n_c+3,N);
		linear_map(Glob,-1.0,B1,Sol);  // kappa*(<u>-u)
		vector_add(F,Glob,kappa*dt,N*n_mesh);
		free(dT);
		free(Glob);
		free_sparse(B1);*/
...
void init_system_matrix(void* Params){
	...
	//set_partition_mode(PARTITION_ON);
	Factors[0] = 1.;
	Factors[1] =(1.-front_factor*gel)/sol;
	...
	//sol
	sparse_matrix* L = set_matrix_Aij_11_2D(n_v-o,n_v-o,&insert_A_bbV_a);
	sparse_add(System_A,L,-eta_eff);
	...
	set_partition_mode(PARTITION_ON);
	sparse_matrix* L1 = set_matrix_Aij_11_2D(n_c,n_c,&insert_A_bbV);
	sparse_add(System_B,L1,D_c*dt);
	set_partition_mode(PARTITION_OFF);
	
	sparse_matrix* L2 = set_matrix_Aij_11_2D(n_c+1,n_c+1,&insert_A_bbV);
	...
	if (rootnum>0){
		char fix_name[512];
		sprintf(fix_name,"%s/%s",Mesh_Dir,Output_name);
		...
		for (i=0;i<rootnum;i++){
			...
			Fixpoints[i][3] = slow_fTa(Fixpoints[i],0);
			//Fixpoints[i][1] = Fix[i][0];
			//Fixpoints[i][2] = Fix[i][0];
			//Fixpoints[i][3] = -KT*Fix[i][0]/(1.+Fix[i][0]);	// Achtung nur ohne R.-kinetik !
	...
	if (strcmp(load_matrix,"none")!=0){							// Achtung: System_A hängt von dt ab !
		sprintf(dir,"%s/%s/LUinfo",Mesh_Dir,load_matrix);
		...
		sprintf(dir,"%s/%s/LUinfo",Mesh_Dir,Output_name);
		mkdir(dir,0777);
...
void mechanical_part(double* Solution,double* Old_Solution,double t,double dt){
	...
	if (Fixpoints!=NULL) vector_shift(T,4*n_mesh,-Fixpoints[0][n_c+3]);
	...
	Factors[0] = 1.;
	Factors[1] = front_factor;
	//sparse_matrix* G1 = set_matrix_Aij_10_2D(0,0,&insert_A_aV);
...
void get_params(char* path,int argc,char* argv[],int param_num){
	...
	if (name!=NULL){
		...
		char p_name[512];
		sprintf(p_name,"%s/%s",Mesh_Dir,Output_name);
		if (mkdir(p_name,0777)==-1){
			printf("File %s existiert bereits -> weiter ...\n",p_name);
			Reload = 1;
		}
...
int main(int argc, char* argv[]){
	...
	get_params("/home/radszuweit/Daten",argc,argv,param_number);
	...
	run_FEM_simulation(Output_name,Mesh_name,Mesh_Dir,step_number,data_interval,
	 total_time,Reload);
	free_look_up_tables();
